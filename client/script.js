...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Hello-again').then((response) => {console.log(response.data)
...
// CODE HERE
function sendQuery() {
    axios.get('http://localhost:3000/query-test/?name=Laura').then((result) => {console.log(result.data)
    const querySent = document.getElementById('query-button')
    querySent.textContent = result.data
    querySent.style.display = 'bold'
})
}
